...
CREATE TABLE [control].[table_metadata] (
    ...
    [silver_path] VARCHAR(500) NOT NULL,
    [load_priority] INT NOT NULL DEFAULT 99,
    [created_date] DATETIME2 NOT NULL DEFAULT GETDATE(),
    [modified_date] DATETIME2 NOT NULL DEFAULT GETDATE(),
    CONSTRAINT [FK_table_metadata_source_systems] FOREIGN KEY ([source_system_id]) REFERENCES [control].[source_systems]([source_system_id])
);
GO
...
CREATE TABLE [control].[load_dependencies] (
    ...
    CONSTRAINT [FK_load_dependencies_table] FOREIGN KEY ([table_id]) REFERENCES [control].[table_metadata]([table_id]),
    CONSTRAINT [FK_load_dependencies_depends_on] FOREIGN KEY ([depends_on_table_id]) REFERENCES [control].[table_metadata]([table_id])
);
GO
...
CREATE TABLE [control].[pipeline_execution_log] (
    ...
    CONSTRAINT [FK_pipeline_execution_log_table] FOREIGN KEY ([table_id]) REFERENCES [control].[table_metadata]([table_id])
);
GO
...
CREATE TABLE [control].[data_quality_rules] (
    ...
    [is_active] BIT NOT NULL DEFAULT 1,
    CONSTRAINT [FK_data_quality_rules_table] FOREIGN KEY ([table_id]) REFERENCES [control].[table_metadata]([table_id])
);
GO